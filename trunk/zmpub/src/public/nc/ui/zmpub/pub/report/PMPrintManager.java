...
public class PMPrintManager extends nc.ui.report.base.PrintManager { // static
																		// int
																		// m_iContentRowCount
																		// =0;
	...
	private static int[] getAlignFlag(Object obj[][]) {
		...
		if (obj != null && obj.length != 0) {
			...
			for (int i = 0; i < len; i++) {
				...
				do {
					if (obj[row][i] == null
							|| obj[row][i].toString().length() == 0)
						continue;
					if ((obj[row][i] instanceof UFDouble)
							|| (obj[row][i] instanceof Integer)
							|| (obj[row][i] instanceof Double))
						alignFlag[i] = 2;
					else if ((obj[row][i] instanceof String)
							&& obj[row][i].toString().equals("√"))
						alignFlag[i] = 1;
					else
						alignFlag[i] = 0;
					break;
		...
		for (int x = 0; x < width - 1; x++) {
			for (int y = 0; y < len; y++)
				if (colname[x][y] != null) {
					...
					if (xb != xe || yb != ye)
						rangeVector.addElement(new CellRange(xb, yb, xe - 1,
								ye - 1));
				}
	...
	public static PrintDirectEntry getDirectPrinter(
			nc.ui.pub.bill.BillCardPanel reportBase, String title) {
		...
		// printer.setMargin(25 ,25, 25, 25);//top ,left ,bottom, right
		// 此处设置有问题跟列宽有关
	...
	/**
	 * 此处插入方法说明。 创建日期：(2002-5-30 14:20:25)
	 */
	public static String getItemPrintValue(BillItem item) {
		...
		case BillItem.DECIMAL: {
			...
			nc.vo.pub.lang.UFDouble ufd = new nc.vo.pub.lang.UFDouble(wb, -item
					.getDecimalDigits());
		...
		case BillItem.USERDEF: {
			if (item.getComponent() instanceof UIRefPane)
				strValue = ((UIRefPane) item.getComponent()).getRefModel()
						.getRefNameValue();
			// strValue = ((UIRefPane)item.getComponent()).isReturnCode() ?
			// ((UIRefPane)item.getComponent()).getRefCode() :
			// ((UIRefPane)item.getComponent()).getRefName();
			break;
		}
		case BillItem.UFREF: {
			if (item.getComponent() instanceof UIRefPane)
				// strValue = ((UIRefPane)item.getComponent()).getRefPK();
				strValue = ((UIRefPane) item.getComponent()).isReturnCode() ? ((UIRefPane) item
						.getComponent()).getRefCode()
						: ((UIRefPane) item.getComponent()).getRefName();
			break;
		}
		case BillItem.COMBO: {
			if (item.getComponent() instanceof UIComboBox)
				if (((UIComboBox) item.getComponent()).getSelectedItem() != null)
					// if (item.isWithIndex())
					// strValue =
					// ((UIComboBox)item.getComponent()).getSelectedIndex() +
					// "";
					// else
					strValue = ((UIComboBox) item.getComponent())
	...
	/**
	 * 此处插入方法说明。 创建日期：(2004-08-26 16:44:14)
	 * 
	 * @param reportBase
	 *            nc.ui.pub.report.ReportBaseClass
	 */
	public static void setBody(nc.ui.pub.bill.BillCardPanel reportBase,
	 ...
	 *            nc.ui.pub.report.ReportBaseClass
	 */
	public static void setBottom(nc.ui.pub.bill.BillCardPanel reportBase,
			PrintDirectEntry printer) {
			...
			if (itemCount > 0) {
				...
				for (int i = 0; i < itemCount; i++) {
					...
					if (bodyItems[i].getDataType() == nc.ui.pub.bill.BillCardPanel.UFREF)
						value = ((UIRefPane) bodyItems[i].getComponent())
								.getText();
					else
						value = bodyItems[i].getValue();

					headStr[i / 2][i % 2] = bodyItems[i].getName() + ": "
							+ value;
					headHeight[i / 2] = 15;
				}
	...
	/**
	 * 简单处理：表头只打印两列 创建日期：(2004-08-26 16:44:14)
	 * 
	 * @param reportBase
	 *            nc.ui.pub.report.ReportBaseClass
	 */
	public static void setHead(nc.ui.pub.bill.BillCardPanel reportBase,
			PrintDirectEntry printer) {
		int height = 15;
		...
		if (headItemCount > 0) {
			...
			for (int i = 0; i < headItemCount; i++) {
				headStr[i / 2][i % 2] = headItems[i].getName() + ": "
						+ headItems[i].getValue();
	 ...
	 * 此处插入方法说明。 创建日期：(2002-5-30 14:20:25)
	 */
	public static void setPrintTopPub(
			nc.ui.pub.bill.BillCardPanel billcardpanel,
			nc.ui.pub.print.PrintDirectEntry print) {
		int ie = 0; // debug
		int je = 0; // debug
		try {
			// 得到表头信息
			...
			for (int i = 0; i < headlinecount; i++) {
				...
				for (j = 0; j < headrowcount * 2; j++) {
					if (j % 2 == 0) {
						strTops[i][j] = headitems[i * headrowcount + j / 2]
								.getName()
								+ ":"
								+ headitems[i * headrowcount + j / 2]
										.getValue();
					}
					// else {
					// strTops[i][j] = getItemPrintValue(headitems[i *
					// headrowcount + (int) j / 2]);
					// }
