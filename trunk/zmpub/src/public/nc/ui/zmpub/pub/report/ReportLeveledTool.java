...
/**
 * 工具类：构造分级统计VO. 创建日期：(2003-11-13 22:33:57)
 * 
 * @author：刘建波
 */
public class ReportLeveledTool {
	...
	public class SumVisitor {
		...
		public void addDetail(nc.vo.pub.CircularlyAccessibleValueObject voTotal) {
			...
			// 将合计的明细汇总。
			java.util.Vector vDetailOnTotal = ((IReportVO) voTotal)
					.getDetailvo();
			if (vDetailOnTotal != null && vDetailOnTotal.size() > 0) {
				...
				vDetail
						.add((nc.vo.pub.CircularlyAccessibleValueObject) voTotal);
		...
		public void sum(nc.vo.pub.CircularlyAccessibleValueObject voTotal,
				nc.vo.pub.CircularlyAccessibleValueObject voDetail) { // 初始voTotal.
			if (voTotal == null) {
				if (voDetail != null)
					voTotal = (nc.vo.pub.CircularlyAccessibleValueObject) voDetail
							.clone();
				else
					return;
			} // 初始化m_resultVO.
			if (getResult() == null) {
				if (voTotal != null) {
					m_resultVO = (nc.vo.pub.CircularlyAccessibleValueObject) voTotal
							.clone();
					// addDetail(voTotal)将voTotal的Detail复制到新的Vector;
					((IReportVO) m_resultVO)
							.setDetailVO(new java.util.Vector());
					addDetail(voTotal);
				}
				return;
			}
			for (int i = 0; i < getFields().length; i++) {
				...
				case IUFTypes.INT:
					...
					int itmp = (tmpobj == null ? 0 : ((Integer) tmpobj)
							.intValue());
					getResult().setAttributeValue(getFields()[i],
				...
				case IUFTypes.LONG:
					...
					continue;
				case IUFTypes.UFD:
					nc.vo.pub.lang.UFDouble ufdtmp = (tmpobj == null ? new nc.vo.pub.lang.UFDouble(
							"0")
							: (nc.vo.pub.lang.UFDouble) tmpobj);
					nc.vo.pub.lang.UFDouble ufdResult = (resultobj == null ? new nc.vo.pub.lang.UFDouble(
							"0")
	...
	private ExTreeNode combinDown(ExTreeNode node) {
		SumVisitor visitor = new SumVisitor(getSumInfoVO(),
				m_isCheckDetailIterative);
		// 调整NodeVO.
		m_voLevelInfo.getAdjustNodeTool().adjustNodeVO(node);
		if (!node.isLeaf()) {
			for (int i = 0; i < node.getChildObjtypecode().length; i++) {
				SumVisitor visitorTmp = new SumVisitor(getSumInfoVO(),
						m_isCheckDetailIterative);
				for (int j = 0; j < node.getChildNodeByObjcode(node
						.getChildObjtypecode()[i]).length; j++) {
					ExTreeNode nodetmp = node.getChildNodeByObjcode(node
							.getChildObjtypecode()[i])[j];
				...
				if (visitorTmp.getResult() != null) {
					if (node
							.getSubTotalVOByObjcode(node.getChildObjtypecode()[i]) == null) {
						node.setSubtotalVOByObjcode(
								node.getChildObjtypecode()[i],
								(CircularlyAccessibleValueObject) visitorTmp
										.getResult().clone());
					}
					((IReportVO) node.getSubTotalVOByObjcode(node
							.getChildObjtypecode()[i])).setSubtotal(true);
					m_voLevelInfo.getAdjustNodeTool().adjustSubtotalVO(node,
							node.getChildObjtypecode()[i]);
				}
			}
			// 设置合计VO.
			...
			if (visitor.getResult() == null)
				return node;
			node.setTotalVO((CircularlyAccessibleValueObject) visitor
					.getResult());
	...
	public nc.vo.pub.CircularlyAccessibleValueObject[] getLevelReportVO(
			LevelByOrgContext voLevelinfo, SumInfoVO voSumInfo,
		...
		/***
		 * 对每一个节点向下递归汇总。 对每一个非末级节点初始合计小计的VO.
		 * 对于非末级节点进行小计合计的设置。合计表为：此节点的名称＋（小计）合计。 合计的值为所有的子节点的和＋本身发生额。
		 * 节点要实例自己的DetailVO.
		 */
		 ...
		 * 得到每一个选择节点的VO[].
		 */
		nc.vo.pub.CircularlyAccessibleValueObject[][] voss = getLevelvoByNodes(topnodes);
		// 对VO[]进行合并同时加小计合计。
	...
	/**
	 * 此处插入方法说明。 创建日期：(2003-11-15 11:42:36)
	 * 
	 * @return nc.vo.mbt.report.AcctReportVO[][]
	 * @param nodes
	 *            [i]s nc.ui.mbt.util.ExTreeNode
	 */
	private CircularlyAccessibleValueObject[][] getLevelvoByNodes(
			ExTreeNode[] nodes) {
	...
	private Vector getLevelvoByNodes(ExTreeNode node, Vector v) {
		...
		for (int i = 0; i < node.getChildObjtypecode().length; i++) {
			...
			for (int j = 0; j < node.getChildNodeByObjcode(node
					.getChildObjtypecode()[i]).length; j++) {
				ExTreeNode tmp = node.getChildNodeByObjcode(node
						.getChildObjtypecode()[i])[j];
				if (!tmp.isLeaf()) {
					getLevelvoByNodes(tmp, v);
				}
				if (tmp.getNodeVO() != null && tmp.isLeaf()
						&& isShow(tmp, NODEVO))
					v.add(tmp.getNodeVO());
			}
			if (node.getSubTotalVOByObjcode(node.getChildObjtypecode()[i]) != null
					&& !node.isLeaf()
					&& m_voLevelInfo.isShowSubtotal()
					&& isShow(node, SUBTOTALVO))
				v.add(node
						.getSubTotalVOByObjcode(node.getChildObjtypecode()[i]));
		}
		if (node.istopSelected() && node.getNodeVO() != null && node.isLeaf())
			v.add(node.getNodeVO());
		if (node.getTotalVO() != null && !node.isLeaf()
				&& isShow(node, TOTALVO))
	...
	private CircularlyAccessibleValueObject getTotalVO(ExTreeNode[] nodes) {
		...
		if (visitor.getResult() != null) {
			m_voLevelInfo.getAdjustNodeTool().adjustAllTotalVO(
					visitor.getResult());
	...
	/*
	 * vo是否显示的判断。 加了是否全不为0是否显示的判断。 现在的策略是：如果subtotal,totalvo都为0 。则此节点不显示。
	 * 可能要改成每一个VO的粒度而不是现在的节点。 是否只是显示最高节点也加入了。
	 */
	private boolean isShow(ExTreeNode node, int iVOtype) {
		/**
		 * 如果级次为空,则为全选 是否在显示的级次内.
		 */
		boolean isInLevel = m_voLevelInfo.getShowLevels() == null
				|| Toolkit.binarySearch(m_voLevelInfo.getShowLevels(), Integer
						.toString(node.getLevel())) > -1;
		if (!isInLevel)
			return false;
		// 是否显示明细.
	...
	private boolean visitForZero(ExTreeNode node) {
		...
		for (int i = 0; i < m_voSumInfo.getFields().length; i++) {
			...
			case IUFTypes.INT:
				if ((nodeVO == null || ((Integer) nodeVO
						.getAttributeValue(m_voSumInfo.getFields()[i]))
						.intValue() == 0)
						&& (totalVO == null || ((Integer) totalVO
								.getAttributeValue(m_voSumInfo.getFields()[i]))
								.intValue() == 0)) {
					tmp = true;
					continue;
			...
			case IUFTypes.LONG:
				if ((nodeVO == null || ((Long) nodeVO
						.getAttributeValue(m_voSumInfo.getFields()[i]))
						.longValue() == 0
						&& (totalVO == null || ((Long) totalVO
								.getAttributeValue(m_voSumInfo.getFields()[i]))
								.longValue() == 0))) {
					tmp = true;
					continue;
			...
			case IUFTypes.UFD:
				if ((nodeVO == null
						|| nodeVO.getAttributeValue(m_voSumInfo.getFields()[i]) == null || ((UFDouble) nodeVO
						.getAttributeValue(m_voSumInfo.getFields()[i]))
						.equals(new UFDouble("0")))
						&& (totalVO == null
								|| totalVO.getAttributeValue(m_voSumInfo
										.getFields()[i]) == null || ((UFDouble) totalVO
								.getAttributeValue(m_voSumInfo.getFields()[i]))
								.equals(new UFDouble("0")))) {
					tmp = true;
					continue;
			...
			for (int k = 0; k < arsSubObjcode.length; k++) {
				if (node.getSubTotalVOByObjcode(arsSubObjcode[k]) == null
						|| node.getSubTotalVOByObjcode(arsSubObjcode[k])
								.getAttributeValue(m_voSumInfo.getFields()[i]) == null
						|| node.getSubTotalVOByObjcode(arsSubObjcode[k])
								.getAttributeValue(m_voSumInfo.getFields()[i])
