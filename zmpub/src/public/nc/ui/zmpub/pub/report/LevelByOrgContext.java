...
public class LevelByOrgContext {
	...
	public javax.swing.tree.DefaultTreeModel getCombinedTM() {
		...
		ExTreeNode root = (ExTreeNode) getModelByObjcode(
				getObjcodeByLevel(0)[0]).getRoot();
		for (int i = 1; i < getLevelNumber(); i++) {
			for (int j = 0; j < getObjcodeByLevel(i).length; j++) {
				Hashtable hnode = null;
				String strParentCode = ((ExTreeNode) getModelByObjcode(
						getObjcodeByLevel(i)[j]).getRoot()).getParentCode();
				hnode = root.getAllNodeByObjtypecode(null, strParentCode);
				// 得到下一个树的所有根节点。
				Enumeration en = ((ExTreeNode) getModelByObjcode(
						getObjcodeByLevel(i)[j]).getRoot()).children();
				// 叶子对下一个树的根进行合并。
				while (en.hasMoreElements()) {
					ExTreeNode downtop = (ExTreeNode) en.nextElement();
					ExTreeNode supernode = (ExTreeNode) hnode.get(downtop
							.getSuperobjid());
	...
	public nc.vo.pub.CircularlyAccessibleValueObject getDataByObjcode(
			String strObjcode) {
		return (nc.vo.pub.CircularlyAccessibleValueObject) m_hObjData
				.get(strObjcode);
	}
	...
	public javax.swing.tree.DefaultTreeModel getModelByObjcode(String strObjcode) {
		return (javax.swing.tree.DefaultTreeModel) m_hObjTreeModel
				.get(strObjcode);
	}
	...
	private void initTMByData(String strObjcode,
		...
		if (isUsecodeMatchData())
			hnode = ((ExTreeNode) getModelByObjcode(strObjcode).getRoot())
					.getAllNodeByObjtypecodeObjCodeAsKey(null, strObjcode);
		else
			hnode = ((ExTreeNode) getModelByObjcode(strObjcode).getRoot())
					.getAllNodeByObjtypecode(null, strObjcode);
	...
	public void initTMByObjcode(String[][] objcode) {
		...
		for (int i = 1; i < objcode.length; i++) {
			for (int j = 0; j < objcode[i].length; j++) {
				m_hObjTreeModel.put(objcode[i][j], getTMFactory().getModel(
						objcode[i][j]));
			}
		}
	}
	...
	public void setDataByObjcode(String strObjcode,
			nc.vo.pub.CircularlyAccessibleValueObject[] vos) {
		// 初始树模。
